    ...
    def __call__(self, x):
        ...
        for i, func in enumerate(self.functions):
            if isinstance(func, L.Deconvolution2D):
                ...
                print(f'function {i}, {func} is called in else!')
                h = func(h)
        return h



class ResnetGeneratorWithSkip(chainer.Chain):

    def __init__(self, intermediates=9, skip=True):
        self.skip = skip
        if intermediates > 9:
            raise ValueError(f"Arg intermediates must be not larger than 9, but given {intermediates}")
        super(ResnetGeneratorWithSkip, self).__init__()
        ...
        with self.init_scope():
            ...
            self.l20 = InstanceNormalization(128, decay=0.9, eps=1e-05)
            self.l21 = lambda x: F.relu(x)
            self.l22 = L.Deconvolution2D(None, 64, ksize=3, stride=2, pad=1,
                                         initialW=initialW)
            self.l23 = InstanceNormalization(64, decay=0.9, eps=1e-05)
            self.l24 = lambda x: F.relu(x)
            self.l25 = lambda x: F.pad(x, [(0, 0), (0, 0), (3, 3), (3, 3)],
        ...
        for i in range(0, 10):
            self.functions.append(getattr(self, 'l{:d}'.format(i)))
        for i in range(intermediates):
            self.functions.append(getattr(self, 'l{:d}'.format(i+10)))
        for i in range(10+intermediates, 28):
            self.functions.append(getattr(self, 'l{:d}'.format(i)))
